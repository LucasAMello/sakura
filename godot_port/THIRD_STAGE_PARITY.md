...
Entry and reward portal presentation uses the shared source-matched implementation. Map geometry, state order, per-update movement values, damage, and spawn data follow the canonical source, with enemy-side updates paced by the confirmed 30 Hz correction above. Canonical music, sound effects, menus, JSON and legacy saving, pause weapon selection, and control remapping are provided by the shared project systems.
