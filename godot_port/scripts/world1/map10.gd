...
enum StageState { ENTRY, PLAYING, EXITING, COMPLETE, DYING, INTERMISSION_ENTRY, INTERMISSION, BOSS_INTRO, BOSS, VICTORY }

@export var map_number := 14
...
func _ready() -> void:
	...
	terrain.setup("res://maps/map%d.map" % map_number, map_config["width"], map_config["height"])
	terrain.z_index = 0
	add_child(terrain)
...
func _update_entry() -> void:
	...
	var has_portal := map_number == 10 or (map_number == 14 and intermission_active)
	if state_ticks >= 130 or (!has_portal and state_ticks >= 86):
		...
		stage_state = StageState.INTERMISSION if intermission_active else StageState.PLAYING
		state_ticks = 0
...
func _update_exit() -> void:
	...
	if next_map > 0:
		...
		stage_state = StageState.COMPLETE
		hud.show_message("FIRST STAGE COMPLETE\n\nPress R to replay map14")
...
func _on_boss_reward_collected() -> void:
	...
	departure_ticks = 1
	departure_walk_direction = player.facing
	departure_target_player_position = player.position + Vector2(departure_walk_direction * 40.0, 0.0)
	departure_portal_center = departure_target_player_position + SakuraPlayer.BODY_SIZE * 0.5
	player.set_scripted_animation_active(true)
...
func _update_departure() -> void:
	...
	if departure_ticks < 16:
		_set_departure_frame(preload("res://assets/player/portal_3.png"))
	elif departure_ticks < 24:
		_set_departure_frame(preload("res://assets/player/portal_4.png"))
	elif departure_ticks < 32:
		_set_departure_frame(preload("res://assets/player/portal_5.png"))
	elif departure_ticks < 40:
		_set_departure_frame(preload("res://assets/player/portal_6.png"))
		...
		if departure_ticks < 80:
			var remaining := (departure_target_player_position.x - player.position.x) * departure_walk_direction
			player.scripted_step(departure_walk_direction, maxf(0.0, minf(2.0, remaining)))
		else:
...
func _set_departure_frame(texture: Texture2D) -> void:
	departure_back.visible = false
	departure_front.visible = true
...
func _entry_should_show_ready() -> bool:
	return map_number == 10 or (map_number == 14 and intermission_active)
