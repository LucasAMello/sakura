...
func _update_idle_animation() -> void:
	timer += 1
	if timer % 3 == 0:
		sprite.texture = IDLE_FRAMES[int(timer / 3.0 - 1.0) % IDLE_FRAMES.size()]


func _update_intro() -> void:
	timer += 1
	if timer == 2 or timer == 22:
...
func _update_hadouken() -> void:
	timer += 1
	if timer == 1 or timer == 28:
		sprite.texture = FIRE_FRAMES[0]
	elif timer == 3 or timer == 26:
		sprite.texture = FIRE_FRAMES[1]
	elif timer == 5:
		sprite.texture = FIRE_FRAMES[2]
	elif timer == 20:
