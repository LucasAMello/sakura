...
SFX mix update (2026-09-14): the user requested approximately -10 dBFS individual sound peaks and 20% SFX Options steps. Preserve `SFX_GAIN_DB` in `audio_manager.gd`, including gains above unity for quiet recordings, and both menu adjustment paths using `SFX_VOLUME_STEP`. These per-effect adjustments supersede archive relative loudness; the quieter contextual fireyflame/anim13 multipliers remain. See `godot_port/SFX_LEVELS.md`. This change does not authorize modifying source WAVs, import compression, music settings, or gameplay timing.
...
- Audio update (2026-09-14): the user confirmed seamless CSelect playback with a native loop offset into its unchanged base OGG and requested this for all looping music. All ten looping tracks now use `MUSIC_LOOP_OFFSETS` in `audio_manager.gd`; separate loop and composite OGGs were removed. Preserve this user-approved approach and existing initial playback cues. It supersedes the old requirement for two fresh-state renders. Derive offsets from the MIDI tempo map relative to the existing render's initial seek, rounded to the nearest 44,100 Hz sample; see `godot_port/AUDIO_PARITY.md`. Do not restore split renders or shift markers based on an unverified Allegro beat-number interpretation.
